function instrument(freq, gainAmount)
{
  return baseTrack(
    sine(freq)
      .then(gain(gainAmount || 0.5))
...
function exp2(v) { return Math.exp(v * Math.log(2)); }

function instrument2(freq, opts)
{
...
function makeNote(baseFreq, centsAdjustment, opts)
{
...
d3.select("#go").append("button").text("just").on("click", () => {
  player.playTrack(overlappingSeq(
    [makeNote(261.63, 0),
     ...
     makeNote(261.63, ratioToCents(3/2))], 0.025)); // just perfect fifth
});
...
d3.select("#go").append("button").text("equal").on("click", () => {
  player.playTrack(overlappingSeq(
    [makeNote(261.63, 0),
     ...
     makeNote(261.63, 700)], 0.025)); // equal perfect fifth
});

d3.select("#go").append("button").text("just major third").on("click", () => {
  player.playTrack(overlappingSeq(
    [makeNote(261.63, ratioToCents(4/3))], 0.05)); // just perfect fifth
});

d3.select("#go").append("button").text("equal major third").on("click", () => {
  player.playTrack(overlappingSeq(
    [makeNote(261.63, 500)], 0.05)); // equal perfect fifth
});

d3.select("#go").append("button").text("chromatic scale in quartertones").on("click", () => {
  let seq = [];
  for (let i = 0; i <= 12; ++i) {
...
d3.select("#go").append("button").text("Microchromatic scale in quartertones").on("click", () => {
  let seq = [];
  for (let i = 0; i <= 24; ++i) {
...
d3.select("#go").append("button").text("C minor equal").on("click", () => {
  player.playTrack(overlappingSeq([0, 300, 700].map(cent => makeNote(261.63, cent)), 0.05));
});

d3.select("#go").append("button").text("C major equal").on("click", () => {
  player.playTrack(overlappingSeq([0, 400, 700].map(cent => makeNote(261.63, cent)), 0.05));
});

d3.select("#go").append("button").text("C neutral 350c").on("click", () => {
  player.playTrack(overlappingSeq([0, 350, 700].map(cent => makeNote(261.63, cent)), 0.05));
});

function getHarmonicFreqs(baseNote, count)
{
